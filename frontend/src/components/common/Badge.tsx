...
export interface BadgeProps extends HTMLAttributes<HTMLSpanElement> {
  variant?: 'rose' | 'emerald' | 'amber' | 'sky' | 'neutral' | 'indigo' | 'slate' | 'cyan';
  pulse?: boolean;
}
...
export const Badge: React.FC<BadgeProps> = ({ className, variant = 'rose', pulse = false, children, ...props }) => {
  const variants = {
    ...
    indigo: 'bg-indigo-500/15 text-indigo-300 border-indigo-500/30 shadow-sm shadow-indigo-950/30',
    slate: 'bg-slate-800/80 text-slate-300 border-slate-700',
    cyan: 'bg-cyan-500/15 text-cyan-300 border-cyan-500/30 shadow-sm shadow-cyan-950/30',
  };
  ...
  return (
    <span
      ...
      {...props}
    >
      {pulse && <span className={cn('w-1.5 h-1.5 rounded-full animate-pulse', dotColors[variant] || 'bg-rose-400')} />}
      {children}
    </span>
  );
};
